...
cmd="$PYTHON_CMD dhtserver.py --restserver $PUBLIC_IP --dhtserver $PUBLIC_IP --ksize $KSIZE --alpha $ALPHA --dhtport $MAIN_SERVER_PORT --dhtdbpath $DB_PATH/mainnode --store_state_file $STATE_PATH_FILE"
...
do
    ...
    cmd="$PYTHON_CMD dhtserveronly.py --dhtserver $PUBLIC_IP --ksize $KSIZE --alpha $ALPHA --bootstrap $PUBLIC_IP:$MAIN_SERVER_PORT --dhtdbpath $DB_PATH/node$c --store_state_file $STATE_PATH_FILE --dhtport $CUR_PORT"
    if [ -d "$STATE_PATH_FILE" ]; then
        cmd="$cmd --dht_cache_file $STATE_PATH_FILE"
    fi
